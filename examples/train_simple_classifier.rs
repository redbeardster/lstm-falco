// Offline demo: single-layer sigmoid classifier on aggregated sequence features.
// NOT compatible with production `data/lstm_model.json` (LSTM BPTT format).
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Starting simple classifier training (not production LSTM)...");
    ...
    // Сохраняем модель
    let model_path = "data/simple_classifier_model.json";
